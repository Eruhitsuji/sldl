...
このページでは、v1.0.5で推奨するワークフローを説明します。新規文書では、schema-bound templateから文書とproject JSONを生成し、project commandで各形式へ出力する流れが基本です。
...
`template explain` では、テンプレート本体、document type、紐づいたschema、export label config、LaTeX build config、strict-schema設定を確認できます。v1.0.5では `text`、`markdown`、`json` の出力形式に対応しています。`templates/template_manifest.json` が正式なmanifestで、`templates/manifest.json` は互換用コピーです。
