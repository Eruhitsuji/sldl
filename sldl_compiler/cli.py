...
def _template_docs_output(template_dir: str | None, fmt: str, language: str = "en") -> str:
    ...
    if(language=="ja"):
        lines=["# SLDL Template Reference（日本語）", "", "同梱template manifestから生成したテンプレート一覧です。v1.0.16では、template referenceとdiagnostics referenceに加えて、reference indexとCLI help referenceもrelease checkで差分確認できます。", "", "| Name | Document type | Language | Schema | Role |", "|---|---|---|---|---|"]
    else:
        lines=["# SLDL Template Reference", "", "Generated from the bundled template manifest. In v1.0.16, release checks keep the template and diagnostics reference drift checks and also drift-check the reference index and CLI help reference.", "", "| Name | Document type | Language | Schema | Role |", "|---|---|---|---|---|"]
    for item in display_items:
...
def _make_template_project_config(args, tmpl) -> dict:
    ...
    config={
        ...
        "description": f"SLDL v1.0.16 project generated from template: {tmpl.name}",
        "version": "1.0.16",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser=argparse.ArgumentParser(prog="sldlc", description="SLDL v1.0.16 compiler")
