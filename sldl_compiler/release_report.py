...
def _category_summary(checks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    categories=sorted({str(item.get("category", "unknown")) for item in checks})
    rows=[]
    for category in categories:
        subset=[item for item in checks if(str(item.get("category", "unknown"))==category)]
        total=len(subset)
        failed=sum(1 for item in subset if(not item.get("ok")))
        rows.append({"category": category, "total": total, "passed": total-failed, "failed": failed})
    return rows
...
def render_release_report_markdown(report: dict[str, Any], language: str = "en") -> str:
    ...
    lines=[
        ...
        f"- checks: `{summary.get('passed', 0)}/{summary.get('total', 0)} passed, {summary.get('failed', 0)} failed`",
        "",
        "## Category summary" if(language!="ja") else "## カテゴリ別サマリー",
        "",
        "| Category | Total | Passed | Failed |" if(language!="ja") else "| カテゴリ | 合計 | 成功 | 失敗 |",
        "|---|---:|---:|---:|",
    ]
    for row in report.get("category_summary", []):
        lines.append(f"| `{row.get('category', '')}` | {row.get('total', 0)} | {row.get('passed', 0)} | {row.get('failed', 0)} |")
